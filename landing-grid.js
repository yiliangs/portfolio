...
function attempt(seed, cols, rows, count, reserved, contact, vocab, first, tries, pins) {
...
function readPins(pins, count, cols, reserved) {
...
export function placeLanding(opts) {
  const {
    ...
    vocab = PLATE_VOCAB, first = FIRST_PLATE, tries = 600, maxGrow = 80, pins = {},
  } = opts || {};
  if (!(rows > 0) || !(count >= 0)) throw new Error('landing-grid: placeLanding needs a row count and a plate count');
  const pinned = readPins(pins, count, cols, reserved);
  ...
  for (let r = from; r <= from + maxGrow; r++) {
    const plates = attempt(seed >>> 0, cols, r, count, reserved, contact, vocab, first, tries, pinned);
    if (plates) {
      return {
