  ...
  class Component extends DCLogic {
    ...
    morphTexts(olds) {
      ...
      main.querySelectorAll('[data-shape]').forEach((el) => {
        ...
        // keep the destination blank until the box lands on it, whether that is a slot, a video or
        // the plain <img> the serif chapter shows its hero in (a plain img is not a slot: the query
        // must name it, or the picture sits waiting at the destination under the one flying to it)
        ...
        if (photo) {
          ...
          const slot = el.matches('image-slot, video, img') ? el : el.querySelector('image-slot, video, img');
    ...
    // The lit glyphs come from the effect that lit them rather than back out of the DOM: it knows which chars are
    // lit, how brightly and where they sit, and asking the page instead meant an attribute query and a rect per
    // glyph, taken right after the effect had written that frame's styles, so every breath laid the whole document
    // out again. Hand the roll each glyph's ink box; how finely that box is sampled is the sim's business, not the
    // page's. The box those fractions are against is the one the sim already keeps for its own projection, rather
    // than the layer's rect measured here: the layer is not the viewport (a fixed layer inside the app root takes
    // the root's width, not the window's), and measuring it every frame is the layout this was written to avoid.
    feedParchment() {
      const roll = this.parchLayerRef.current, col = this.heroTextRef.current;
      if (!this.parch || !roll || !col) return;
      const { w: rw, h: rh } = this.parch.layerSize(); if (!rw || !rh) return;
      const out = [];
      for (const t of this.trInstances || []) {
        if (t._destroyed || !col.contains(t.element)) continue;
        for (const g of t.litGlyphs(0.2)) {
          if (!(g.w > 0)) continue;
          ...
          const x = g.x + g.w * 0.15, y = g.y + g.h * 0.3;
          out.push({ x: x / rw, y: y / rh, w: g.w * 0.7 / rw, h: g.h * 0.48 / rh, glow: g.glow });
        }
      }
