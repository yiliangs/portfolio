...
if (mono) {
  ...
  if (!grid) {
    ...
    // Grid items are the placed modules. sc-if / sc-for only wrap them, so look through those.
    const items = [];
    ...
    collect(grid);
    if (!items.length) fail('the sheet grid has no children');

    for (const item of items) {
      if (!declares(item, 'grid-column')) fail('grid item without grid-column: ' + where(item));
      if (!declares(item, 'grid-row')) fail('grid item without grid-row: ' + where(item));

      const margin = declared(item, 'margin');
      if (margin !== null && !/^0(px)?$/.test(margin)) {
        fail('grid item with a margin other than 0 (modules are separated by empty cells, not margins): ' + where(item) + ' has margin:' + margin);
      }
      ...
      if (UA_MARGIN.includes(item.tagName.toLowerCase()) && margin === null && !tight(styleOf(item)).includes('all:unset')) {
        fail('grid item carries the user-agent margin for its tag and never zeroes it: ' + where(item));
      }
    }
...
console.log('check-sheet-grid: the Development sheet is on the 22-column drawing grid and the Research chapter is untouched');
